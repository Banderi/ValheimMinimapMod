...
public class Minimap : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (flag)
		{
			...
			else if (ZInput.GetButtonDown("Map") || ZInput.GetButtonDown("JoyMap") || (m_mode == MapMode.Large && (UnityEngine.Input.GetKeyDown(KeyCode.Escape) || ZInput.GetButtonDown("JoyMap"))))
			{
				switch (m_mode)
				{
				case MapMode.None:
	...
	private void UpdateMap(Player player, float dt, bool takeInput)
	{
		if (takeInput)
		{
			...
			{
				float num = 0f;
				num += UnityEngine.Input.GetAxis("Mouse ScrollWheel") * m_largeZoom * 2f;
		...
		if (m_mode == MapMode.Large)
		{
			...
			{
				m_dragWorldPos = ScreenToWorldPoint(UnityEngine.Input.mousePosition);
				m_dragView = true;
				m_namePin = null;
			}
			...
			if (m_dragView)
			{
				Vector3 vector = ScreenToWorldPoint(UnityEngine.Input.mousePosition) - m_dragWorldPos;
				m_mapOffset -= vector;
	...
	{
		WorldToMapPoint(centerPoint, out float mx, out float my);
	...
	private void UpdatePlayerPins(float dt)
	{
		...
		if (m_playerPins.Count != m_tempPlayerInfo.Count)
		{
			...
			{
				ZNet.PlayerInfo playerInfo2 = item2;
	...
	private void UpdatePingPins()
	{
		...
		if (m_pingPins.Count != m_tempShouts.Count)
		{
			...
			{
				Chat.WorldTextInstance worldTextInstance2 = tempShout;
	...
	private void UpdatePins()
	{
		...
		if (m_mode != MapMode.Large)
		{
			float smallZoom = m_smallZoom;
		}
		else
		{
			float largeZoom = m_largeZoom;
		}
		...
		foreach (PinData pin in m_pins)
		{
			if (IsPointVisible(pin.m_pos, rawImage) && m_visibleIconTypes[(int)pin.m_type] && (m_sharedMapDataFade > 0f || pin.m_ownerID == 0L))
			{
				...
				WorldToMapPoint(pin.m_pos, out float mx, out float my);
	...
	private void UpdatePlayerMarker(Player player, Quaternion playerRot)
	{
		...
		if (m_mode == MapMode.Large && IsPointVisible(position, m_mapImageLarge))
		{
			...
			WorldToMapPoint(position, out float mx, out float my);
		...
		if ((bool)controlledShip)
		{
			...
			if (m_mode == MapMode.Large)
			{
				...
				WorldToMapPoint(position2, out float mx2, out float my2);
	...
	{
		WorldToMapPoint(p, out float mx, out float my);
	...
	private void Explore(Vector3 p, float radius)
	{
		...
		bool flag = false;
		WorldToPixel(p, out int px, out int py);
	...
	{
		WorldToPixel(worldPos, out int px, out int py);
	...
	private void SetMapData(byte[] data)
	{
		...
		{
			ZLog.LogWarning("Missmatching mapsize " + m_mapTexture + " vs " + num2);
			return;
		}
		Reset();
		for (int i = 0; i < m_explored.Length; i++)
		...
		if (num >= 2)
		{
			...
			for (int k = 0; k < num3; k++)
			{
				...
				AddPin(pos, type, name, save: true, isChecked, ownerID);
			}
		}
		if (num >= 4)
		{
	...
	public void OnMapDblClick()
	{
		...
		{
			Vector3 pos = ScreenToWorldPoint(UnityEngine.Input.mousePosition);
	...
	{
		Vector3 vector = ScreenToWorldPoint(UnityEngine.Input.mousePosition);
		Chat.instance.SendPing(vector);
		if (Player.m_debugMode && Console.instance != null && Console.instance.IsCheatsEnabled() && UnityEngine.Input.GetKey(KeyCode.LeftControl))
		{
			Vector3 vector2 = new Vector3(vector.x, Player.m_localPlayer.transform.position.y, vector.z);
			Heightmap.GetHeight(vector2, out float height);
			vector2.y = Math.Max(0f, height);
			Player.m_localPlayer.TeleportTo(vector2, Player.m_localPlayer.transform.rotation, distantTeleport: true);
		}
	}
	...
	private void UpdateBiome(Player player)
	{
		if (m_mode == MapMode.Large)
		{
			Vector3 vector = ScreenToWorldPoint(ZInput.IsMouseActive() ? UnityEngine.Input.mousePosition : new Vector3(Screen.width / 2, Screen.height / 2));
	...
	public bool AddSharedMapData(byte[] dataArray)
	{
		...
		if (num >= 2)
		{
			...
			for (int k = 0; k < num3; k++)
			{
				...
				{
					AddPin(pos, type, name, save: true, isChecked, num4);
					flag4 = true;
				}
			}
		}
